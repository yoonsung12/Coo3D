...
// 겨울 계절 방해 요소인 눈보라를 스폰 범위 내에서 ON/OFF 주기로 내리게 하는 눈구름이다.
// IBlowable을 구현해 선풍기 바람에 밀리고, 바람이 없으면 원래 자리로 복귀한다.
[RequireComponent(typeof(Collider))]
public class SnowController : BaseHazard, IBlowable
{
    [Title("눈보라 ON/OFF 주기")]
    [SerializeField, LabelText("눈 내리는 시간(초)")]
    private float snowOnDuration = 4f;
    // 눈이 내리는 시간이다. 이 시간이 지나면 눈이 멈춘다.

    [SerializeField, LabelText("눈 멈추는 시간(초)")]
    private float snowOffDuration = 3f;
    // 눈이 멈추는 시간이다. 이 시간이 지나면 다시 눈이 내린다.

    [Title("눈송이 설정")]
    [SerializeField, LabelText("눈송이 프리팹")]
    private GameObject snowFlakePrefab;

    [SerializeField, LabelText("눈송이 생성 간격(초)")]
    private float flakeInterval = 0.1f;
    // 값이 작을수록 눈이 더 촘촘하게 내린다.
    ...
    [Title("눈보라 기울기 설정")]
    [SerializeField, LabelText("바람 없을 때 낙하 각도 (도)"), Range(-45f, 45f)]
    private float baseFallAngle = 10f;

    [SerializeField, LabelText("바람에 밀릴 때 추가 기울기 (도)"), Range(0f, 45f)]
    private float windFallAngleBonus = 20f;
    // 구름이 선풍기에 밀리는 동안, 눈송이가 그 방향으로 더 크게 기울어져 "눈보라"처럼 보이게 한다.

    [Title("런타임 상태 (읽기 전용)")]
    [ReadOnly, ShowInInspector, LabelText("현재 밀린 거리")]
    private float _currentOffset;

    [ReadOnly, ShowInInspector, LabelText("눈 내리는 중")]
    ...
    private void UpdateSnowState()
    {
        ...
        float duration = _isSnowing ? snowOnDuration : snowOffDuration;
    ...
    {
        _flakeTimer += Time.deltaTime;

        if (_flakeTimer >= flakeInterval)
        {
            _flakeTimer = 0f;
            GameObject flake = Instantiate(snowFlakePrefab, GetSpawnPosition(), Quaternion.identity);

            // 구름이 바람에 밀리는 중이면, 그 방향으로 눈송이도 더 크게 기울여 눈보라 느낌을 준다.
            bool isBeingBlown = Time.time - _lastBlownTime < BlownCooldown;
            float angle = baseFallAngle + (isBeingBlown ? _pendingPushDirX * windFallAngleBonus : 0f);

            SnowFlake snowFlake = flake.GetComponent<SnowFlake>();
            if (snowFlake != null)
                snowFlake.SetFallAngle(angle);
        }
    }
