...
시점은 3D 사이드뷰(2.5D)이며, 벽점프 등 2D 전용 기믹은 더 이상 사용하지 않는다. 단, 특정 구역(ViewModeZone) 안에서만 탑다운 시점으로 전환된다. 자세한 규칙은 아래 "탑다운 구역(이중 시점) 규칙"을 따른다.
...
기본 시점은 사이드뷰이고, `ViewModeZone` 트리거 구역 안에서만 탑다운으로 바뀐다.

| 항목 | 사이드뷰 (기본) | 탑다운 (ViewModeZone 안) |
|---|---|---|
| 이동 | X(좌우)만, Z 고정 (W/S 무시) | WASD로 X/Z 360도 이동 |
| 점프 | 있음 (공중 점프 포함) | 있음 (공중 점프 포함) |
| 조준 | 마우스, XY 평면 (몸통은 ±90°만 회전) | 마우스, 수평면 XZ (몸통 360° 회전) |
| 카메라 | `SideViewCamera` 사이드뷰 추적 | 같은 `SideViewCamera`의 탑다운 모드 (DOTween 전환) |

* 현재 모드는 `PlayerController.CurrentViewMode`로 판단하고, 모드 변경은 `PlayerController.SetViewMode()`를 통해서만 한다. 카메라는 `OnViewModeChanged` 이벤트로 자동으로 따라간다.
* 플레이어 순간이동은 반드시 `PlayerHealth.Respawn()`을 거친다. 이 메서드가 이동 후 `SyncViewModeToPosition()`으로 시점을 맞춘다. CharacterController를 끈 채 옮기면 `OnTriggerExit`가 오지 않기 때문이다.
* `SideViewCamera`는 보스/퍼즐에서 `Shake()`를 직접 참조하므로 클래스 이름을 바꾸거나 탑다운용 카메라 클래스를 따로 만들지 않는다.
* 보스전은 사이드뷰로 고정한다.
* 탑다운 구역에는 적과 계절 위험물을 두지 않는다. 사이드뷰 전용 오브젝트(적, 위험물 스포너, OneWayPlatform, 우산 퍼즐, LeafMound 등 X축 기준 로직)도 탑다운 구역에 배치하지 않는다. 나중에 필요해지면 탑다운 전용 컴포넌트를 따로 추가하고, 기존 사이드뷰 로직은 건드리지 않는다.
* 탑다운 구역 안에 체크포인트를 둘 경우 해당 `ViewModeZone`의 "복귀 Z 직접 지정"을 켠다.
* 방향/반동 계산을 새로 작성할 때는 두 모드를 모두 고려한다. (예: 사이드뷰는 ±X, 탑다운은 `FacingDirection`의 XZ 성분)
...
## 포트폴리오 기록 규칙 (DEVLOG.md)

이 프로젝트는 나중에 포트폴리오에 쓸 수 있도록, 기능 단위 작업이 끝나고 사용자가 정상 작동을 확인하면 `DEVLOG.md`에 개발 기록을 남긴다. `DEVLOG.md`가 없으면 새로 만든다.

기록 여부는 반드시 사용자에게 먼저 물어보고 승인받은 뒤에만 작성한다. Claude가 임의로 기록을 추가하지 않는다.

기록 작성 절차:

1. 기능 단위 작업이 완료되고 사용자가 정상 작동을 확인하면, "이번 작업을 DEVLOG.md에 기록할까요?" 라고 묻는다.
2. 사용자가 승인하면, Claude는 이번 작업 중 실제로 겪었던 시행착오(에러, 재시도, 접근 방식을 바꾼 이유, 처음 시도가 왜 안 됐는지 등)를 스스로 정리해서 어려웠던 점 초안을 작성한다.
3. 초안을 보여주면서 "혹시 여기에 추가하고 싶은 어려웠던 점이나 직접 겪은 문제가 있나요?" 라고 사용자에게 물어본다.
4. 사용자의 답변을 반영해서 최종 기록을 작성하고 `DEVLOG.md`에 추가(append)한다. 기존 기록은 수정하지 않는다.
...
### 사용 기술
- 이번 작업에서 사용한 기술/API (예: DOTween Sequence, NavMeshAgent, Odin [TabGroup] 등)
- 왜 이 기술을 선택했는지 (다른 대안과 비교했을 때의 이유)

### 어려웠던 점
- Claude가 작업 중 겪은 시행착오 (처음 시도했다가 실패한 방법, 에러 원인, 방향을 바꾼 이유)
- 사용자가 추가로 겪었거나 보완한 어려움

### 결과
- 최종적으로 어떤 기능이 어떻게 완성됐는지 한두 줄 요약
```

기록 시 주의할 것:

* 사소한 오타 수정, 변수명 변경처럼 포트폴리오에 의미 없는 작업은 기록을 제안하지 않는다.
* 새로운 시스템/기능을 추가했거나, 기술적으로 의미 있는 선택(예: CharacterController vs Rigidbody 선택, NavMeshAgent 도입 등)이 있었던 작업 위주로 기록을 제안한다.
* "왜 이 기술을 썼는지"는 실제 프로젝트 맥락(기존 구조 유지, 성능, Inspector 편의성 등)에 근거해서 작성하고, 막연한 이유를 지어내지 않는다.
...
7. 사용자가 정상 작동을 확인하면, 위 "포트폴리오 기록 규칙"에 따라 DEVLOG.md 기록 여부를 물어본다.
...
* 사용자 승인 없이 DEVLOG.md에 기록 추가 금지
* 기존 DEVLOG.md 기록을 임의로 수정하거나 삭제 금지
